...
pacman::p_load(tidyverse, mvtnorm, stringr, MASS, spatstat, GoFKernel, goftest, igraph, 
               tidyr, flowCore, gridExtra, devtools) 

dir_home <- "/Users/zq/qianzhao@umass.edu - Google Drive/My Drive/Research/Histogram/"
dir_work <- "/Users/qianzhao/Documents/Research/BetaTrees/"

# load the BetaTrees package
load_all( path = paste0(dir_work, "BetaTree/R/"))

# load functions 
source(paste0(dir_work,  "BetaTreesPaper/src/compute_marginal.R"))
source(paste0(dir_work,  "BetaTreesPaper/src/summary.R"))

fig_dir <- "/Users/qianzhao/Google Drive/My Drive/Research/Histogram/BetaTree/JASA/V3/Fig/"


data_dir_home <- paste0(dir_home, "V2/Mass Cytometry FSC Live cells gate_Fig4a-g/") 
data_dir_work <- "/Users/qianzhao/Google Drive/My Drive/Research/Histogram/BetaTree/JASA/V2/Mass Cytometry FSC Live cells gate_Fig4a-g/"
data_dir <- data_dir_work
```
...
for(day in seq(2, 12, by = 2)){
  for(trip in 1:3){
    ...
    x <- read.FCS(paste0(data_dir, file_control_name), transformation=FALSE)
...
for(day in seq(2, 12, by = 2)){
  for(trip in 1:3){
    ...
    x <- read.FCS(paste0(data_dir, file_treated_name), transformation=FALSE)
    after <- transform(x, trans) # logicle transformation
    df_new <- as_tibble(after@exprs) %>% 
      dplyr::select(all_of(var_names_all)) %>% 
      add_column(day = day)
...
for(i in 1:3){
  ...
  plot_data_control_marginal <- plot_histogram_data(as.matrix(hist_data_control), 
                                             hist_control, d, 
  ...
  plot_data_treated_marginal <- plot_histogram_data(as.matrix(hist_data_treated), hist_treated, d, 
                                lower_constraint = rep(0.5, d), 
                                upper_constraint = rep(5, d), 
                                plot_coord = c(3, 4), 
                                nint = 50, 
...
```{r marginal_viz, message = F, fig.height=3, fig.width = 4, fig.align="center"}
# Figure 4 in supplement 
for(i in 1:3){
  ...
  labs(title = title, 
    ...
    print(g)
# save figure
# ggsave(g, filename = filename, width = 5, height = 4, units = "in")
  }
}
```
...
for(i in 1:3){
  ...
  plot_data_control_conditional <- plot_histogram_data(as.matrix(hist_data_control),
                                                       hist_control, d, 
  ...
  plot_data_treated_conditional <- plot_histogram_data(as.matrix(hist_data_treated),
                                                       hist_treated, d, 
                                lower_constraint = c(2.170, 2.484, 0.5, 0.5, 0.5), 
                                upper_constraint = c(5, 5, 5, 5, 5), 
                                plot_coord = c(3, 4), 
...
```{r plot_conditional_density, message = F, fig.height=3, fig.width = 4, fig.align="center"}
# Figure 8 in paper 
for(i in 1:3){
  for(type in c("treated", "control")){
    ...
    if(type == "treated"){
      title <- paste0("Period ", i, " Treated - E/M cells" )
    }else{
      title <- paste0("Period ", i, " Control - E/M cells" )
    }
